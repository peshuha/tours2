...
export class TourService implements OnInit, OnDestroy {
  ...
  private sbjTours: ReplaySubject<ITour[] | undefined> = new ReplaySubject<
    ITour[] | undefined
  >(1);
  ...
  private toursPublicate(): void {
    ...
    switch (this.tourType?.type) {
      ...
      case 'multi':
        tours = [
          ...(this.tours as ITour[]).filter(
            (tour) => tour.type === this.tourType?.type
          ),
        ];
        break;
    }
  ...
  findTour(id: string | null): ITour | null {
    ...
    return (
      this.tours.find((tour) => tour.id.toLowerCase() === id.toLowerCase()) ||
      null
    );
  }
